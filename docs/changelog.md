...
- **Weekly maintenance made actually runnable** 2026-07-19: added `upstream.lock` + lockfile-based drift detection (works with an empty `ref/`), reordered the weekly prompt to gates → detect → clone-only-if-drift → triage → update lock, and added 3 acceptance checks (now 55).
